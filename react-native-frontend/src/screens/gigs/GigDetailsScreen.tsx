...
const GigDetailsScreen = () => {
  ...
  const onChangeDate = (event, selectedDate) => {
    ...
    // On iOS, we keep the picker visible
  ...
  const handleBooking = async () => {
    ...
    try {
      await api.post("/bookings", {
        gig_id: gig.id,
        notes: customerNotes,
        booking_time: bookingDate.toISOString(), // Send the selected date
      });
      setShowBookingModal(false);
      Alert.alert(
        "Success",
        "Booking confirmed! You can view it in your dashboard.",
        [
          {
            text: "OK",
            onPress: () => {
              //@ts-ignore
              return navigation.navigate("CustomerTabs", {
                screen: "Bookings",
              });
            },
          },
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        <View className="p-6">
            ...
            {gig.avg_rating && (
              <View className="flex-row items-center gap-1">
                ...
                <Text className="font-bold text-base">
                  {parseFloat(gig.avg_rating).toFixed(1)}
                </Text>
          ...
          <View className="mb-8 p-5 bg-gray-50 rounded-2xl flex-row items-center justify-between">
            ...
            <TouchableOpacity
              onPress={() =>
                navigation.navigate("HandymanProfile", {
                  id: gig.handyman_profile_id || gig.user_id,
                })
              } // Fallback ID
          ...
          <View className="mb-10">
            ...
            {reviews.length === 0 ? (
              ...
              reviews.map((review) => (
                <View
                  key={review.id}
                  className="mb-6 border-b border-gray-50 pb-6 last:border-0"
                >
          ...
          <View className="bg-white rounded-t-3xl p-6 h-[80%]">
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <View className="bg-gray-50 p-4 rounded-xl mb-6">
                <Text className="text-gray-500 font-bold uppercase text-xs tracking-wider mb-1">
                  Service
                </Text>
              ...
              <Text className="text-gray-500 font-bold uppercase text-xs tracking-wider mb-2">
                Schedule Date & Time
              </Text>
              <View className="flex-row gap-4 mb-6">
                <TouchableOpacity
                  onPress={openDatePicker}
                  className={`flex-1 p-4 rounded-xl flex-row items-center justify-between border ${
                ...
                >
                  ...
                  <Feather
                    ...
                    size={20}
                    color={
                      showDatePicker && Platform.OS === "ios" ? "black" : "gray"
                    }
                  />
                </TouchableOpacity>
                ...
                >
                  ...
                  <Feather
                    ...
                    size={20}
                    color={
                      showTimePicker && Platform.OS === "ios" ? "black" : "gray"
                    }
                  />
                </TouchableOpacity>
              </View>
              ...
              {Platform.OS === "ios" && (showDatePicker || showTimePicker) && (
                <View className="bg-gray-50 rounded-2xl mb-6 p-2 border border-gray-200">
                  {showDatePicker && (
                    <DateTimePicker
                      value={bookingDate}
                      mode="date"
              ...
              <TouchableOpacity
                ...
                className="w-full bg-black py-4 rounded-xl items-center mb-4 shadow-lg"
              >
                {bookingSubmitting ? (
